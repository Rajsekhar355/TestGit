...
public class BaseClass extends ReportPortalBaseClass {
	...
	public static WebElement waitForExpectedElement(WebDriver driver, WebElement element) {
		...
		String str = element.toString();
		 ...
		 if (str != null && str.toUpperCase().contains("INPUT") && element.getAttribute("value").equals("---")) {

             try {

                   while (!element.getAttribute("value").isEmpty() && element.getAttribute("type").equals("text")) {
		 ...
		 if (str.contains("//a[@tabindex='-1']") || str.contains("MoreCommandsForContact")) {
               try {
	...
	private static void scrollElement(WebDriver driver, WebElement element) {
		String str = element.toString();
		try {
			if (str != null && element.getAttribute("placeholder").contains("Provide an email")) {
